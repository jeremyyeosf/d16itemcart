...
export class AppComponent implements OnInit {
  ...
  async deleteCart($event: CartItem) {
    console.info('>>> deleteCart: ', $event)

    await this.cartSvc.deleteItem($event.id)
    this.cartContent = await this.cartSvc.getCart()
  }

  async addCart($event: CartItem) {
    console.info('>>> addCart: ', $event)
    await this.cartSvc.addItem($event.id, $event)
    this.cartContent = await this.cartSvc.getCart()
  }
}
